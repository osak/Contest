//Name: NegativeGraphDiv1
//Level: 3
//Category: グラフ,Graph,行列累乗
//Note: SRM626 Div1 Medium

/**
 * 「負辺を1本だけ使ってiからjへ行くときの最適コスト」の距離行列を求め、行列累乗。
 *
 * オーダーは O(N^3 log C + N^2 E)。
 */
#include <iostream>
#include <string>
#include <vector>
...
public:
    ...
    long long findMin( int N, vector <int> from, vector <int> to, vector <int> weight, int charges ) {
        ...
        for(int i = 0; i < E; ++i) {
            const auto a = from[i];
            const auto b = to[i];
            if(dist[a][b] == -1 || dist[a][b] > weight[i]) {
                dist[a][b] = weight[i];
            }
        }
            ...
            for(int i = 0; i < N; ++i) {
                for(int j = 0; j < N; ++j) {
                    if(dist[i][k] != -1 && dist[k][j] != -1) {
                        const auto d = dist[i][k] + dist[k][j];
        ...
        // mat[i][j] = move i to j using most effective negation
        ...
        for(int i = 0; i < N; ++i) {
            for(int j = 0; j < N; ++j) {
                ...
                for(int e = 0; e < E; ++e) {
                    const auto a = from[e];
                    const auto b = to[e];
                    if(dist[i][a] == -1 || dist[b][j] == -1) continue;
                    mat[i][j] = min(mat[i][j], dist[i][a] - weight[e] + dist[b][j]);
                }
            }
        }
        /*
