 ...
 * また、禁止文字列がAGCTとGCTのような場合には、Trieの途中に禁止状態が生じる可能性があるため、使用したすべてのノードを始点として
 * 禁止文字列が含まれているかどうかを調べ、禁止状態フラグを立てておく必要がある。
 *
...
void make_terminal(Node *root, const string &s) {
    const int N = s.size();
    Node *cur = root;
    for(int i = 0; i < N; ++i) {
        const int c = to_idx(s[i]);
        if(!cur->strict[c]) return;
        cur = cur->next[c];
    }
    cur->terminal = true;
}
...
int main() {
    ...
    while(scanf("%d", &N) != -1) {
        ...
        for(int i = 0; i < N; ++i) {
            make_trie(root, strs[i]);
        }
        for(int i = 0; i < N; ++i) {
            for(int j = 0; j < _pos; ++j) {
                make_terminal(&nodes[j], strs[i]);
            }
        }
        while(true) {
            bool changed = false;
