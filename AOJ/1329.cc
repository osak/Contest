...
//Level: 3
//Category: ダイクストラ,Dijkstra,強実装
//Note:

/**
 * 状態としては、王将の位置と周囲4方向のどこが空いているかだけを持てば良い。
 * 遷移は、空いている箇所を違う箇所に移すターン数になるが、これは
 * 歩兵と空きマスを同一視して通行可能とし、幅優先探索することで求められる。
 * 白マス2つをどっちに移すかで、2通りパターンがあることに注意。
 *
 * オーダーはO(H^2W^2 log(HW))。
 */
#include <iostream>
#include <vector>
#include <queue>
